...
function Get-WindowsProductKey {
    try {
        $regKey = Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion" -Name "DigitalProductId" -ErrorAction SilentlyContinue
...
if ($save -eq "y") {
    ...
    if ($type -eq "2") {
        ...
        $PublicReport | Out-File -FilePath $outFile -Encoding UTF8
